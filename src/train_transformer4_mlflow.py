...
SEED, BATCH, EPOCHS, LR, FOLDS = 42, 128, 10, 5e-4, 3
...
def main():
    ...
    for fold,(tr,va) in enumerate(GroupKFold(FOLDS).split(X,y,groups),1):
        ...
        path=models/f"transformer4_fold{fold}.pt"; best=None; best_epoch=0; start=time.perf_counter()

        with mlflow.start_run(run_name=f"transformer4_fold_{fold}"):
    ...
    r.to_csv(out/"transformer4_cv_results.csv",index=False)
    h.to_csv(out/"transformer4_history.csv",index=False)
    p.to_csv(out/"transformer4_oof_predictions.csv",index=False)
