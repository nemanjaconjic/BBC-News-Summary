...
def main():
    ...
    for fold,(tr,va) in enumerate(GroupKFold(FOLDS).split(X,y,groups),1):
        ...
        with mlflow.start_run(run_name=f"transformer1_fold_{fold}"):
            mlflow.log_params({"model":"DistilBERT embeddings + MLP","fold":fold,"batch_size":BATCH,"epochs":EPOCHS,"learning_rate":LR,"hidden":HIDDEN,"dropout":DROPOUT,"folds":FOLDS,"seed":SEED+fold})
            ...
            results.append({"fold":fold,"best_epoch":best_epoch,"accuracy":acc,"precision":pre,"recall":rec,"f1":f1,"val_loss":val_loss,"training_time":train_time,"inference_time":inf,"model_size_mb":size})
    ...
    h.to_csv(out/"transformer1_history.csv",index=False)
    p.to_csv(out/"transformer1_oof_predictions.csv",index=False)

    print("\nTRANSFORMER 1")
    print(r[["fold","accuracy","precision","recall","f1"]].to_string(index=False))
