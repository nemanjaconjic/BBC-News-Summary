...
SEED,BATCH,EPOCHS,LR,FOLDS,HIDDEN,DROPOUT=42,128,10,1e-3,3,256,0.3
...
def evaluate(model,loader,loss_fn):
    ...
    with torch.no_grad():
        for x,y in loader:
            ...
            ys+=y.tolist(); ps+=z.argmax(1).tolist(); probs+=p.tolist()
...
def main():
    seed(SEED); root=Path(__file__).resolve().parent.parent
    ...
    mlflow.set_tracking_uri(f"sqlite:///{(root/'mlflow.db').resolve().as_posix()}"); mlflow.set_experiment("BBC-News-Summary-Transformer")
    out=root/"results/metrics"; models=root/"results/models"; out.mkdir(parents=True,exist_ok=True); models.mkdir(parents=True,exist_ok=True)
    results=[]; history=[]; oof=[]

    for fold,(tr,va) in enumerate(GroupKFold(FOLDS).split(X,y,groups),1):
        seed(SEED+fold)
        train=DataLoader(TensorDataset(X[tr],y[tr]),BATCH,shuffle=True); val=DataLoader(TensorDataset(X[va],y[va]),BATCH)
        model=EmbeddingClassifier(768,HIDDEN,DROPOUT); params=sum(p.numel() for p in model.parameters() if p.requires_grad)
        opt=torch.optim.AdamW(model.parameters(),lr=LR); loss_fn=nn.CrossEntropyLoss()
        path=models/f"transformer1_fold{fold}.pt"; best=None; best_epoch=0; start=time.perf_counter()

        with mlflow.start_run(run_name=f"transformer1_fold_{fold}"):
            ...
            for epoch in range(1,EPOCHS+1):
                ...
                train_loss=total/len(train); val_loss,acc,pre,rec,f1,_,_,_=evaluate(model,val,loss_fn)
                ...
                if best is None or f1>best[4]: best=(val_loss,acc,pre,rec,f1); best_epoch=epoch; torch.save(model.state_dict(),path)

            train_time=time.perf_counter()-start; model.load_state_dict(torch.load(path,weights_only=True))
            ...
            oof += [{"fold":fold,"label":a,"prediction":b,"probability":c} for a,b,c in zip(ys,ps,probs)]
    ...
    r=pd.DataFrame(results); pd.DataFrame(history).to_csv(out/"transformer1_history.csv",index=False); pd.DataFrame(oof).to_csv(out/"transformer1_oof_predictions.csv",index=False)
    r.to_csv(out/"transformer1_cv_results.csv",index=False)
    print("\nTRANSFORMER 1"); print(r[["fold","accuracy","precision","recall","f1"]].to_string(index=False))
