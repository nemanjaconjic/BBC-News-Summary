...
MODEL_NAME = "BaselineModel_V2"


def set_seed(seed):
    """
    Fix random seeds so that experiments are reproducible.
    """
...
def create_groups(df):
    """
    Creates a unique group for every BBC article.

    Sentences from the same article must stay in the same
    fold in order to prevent data leakage.
    """
...
def measure_inference_time(
    ...
    """
    Measures inference time on the validation dataset.
    """
...
def train_fold(
    ...
    """
    Train Model 2 on one cross-validation fold.
    """

    print("\n" + "=" * 60)
    print(f"MODEL 2 - FOLD {fold}")
    print("=" * 60)
    ...
    best_model_path = (
        model_directory
        / f"model2_fold{fold}_best.pt"
    )

    with mlflow.start_run(
        run_name=f"model2_fold_{fold}"
    ):
...
def main():
    ...
    print("\n" + "=" * 60)
    print("MODEL 2 - CROSS-VALIDATION RESULTS")
    print("=" * 60)
